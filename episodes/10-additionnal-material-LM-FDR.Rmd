...
exp <- tibble(gene = "geneX",
       cell = c(rep("cellA", 6), rep("cellB", 6)),
       condition = rep(c(rep("CTL", 3), rep("treated", 3)), 2),
       expression_log = c(rnorm(3, 10, 0.3), rnorm(3, 12, 0.3), rnorm(3, 10, 0.3), rnorm(3, 8, 0.3)))
```
...
```{r echo=FALSE}
exp %>% 
  ggplot(aes(x = condition, y = expression_log)) +
  geom_jitter(size = 3, aes(color = cell)) +
  geom_boxplot(alpha = 0) +
  facet_wrap(~ cell)+
  theme(legend.position = "bottom")
```
