...
set.seed(123)
tiny_dataset <- data.frame(GeneA = round(c(rnorm(10, 30, .5)))) %>%             
  mutate(GeneB = round(c(rnorm(5, 5, 1), rnorm(5, 1000, 100)))) %>%
  mutate(GeneC = round(c(rnorm(5, 80, 5), rnorm(5, 1000, 10)))) %>%
  mutate(GeneD = round(rnorm(10, 500, 10))) %>% 
  mutate(GeneE = round(c(rnorm(5, 800, 5), rnorm(5, 50, 1))))
rownames(tiny_dataset) <- c(paste0("CTL_", 1:5), paste0("Treated_", 1:5))
```
